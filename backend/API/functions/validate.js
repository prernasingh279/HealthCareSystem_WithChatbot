    ...
    switch(method){
        ...
        case 'Doctor-Signup': { //not signup actually, registeration of doctor by admin
            ...
            return [
                ...
                body('fee', 'Fee must be a number. ').optional({checkFalsy:true}).escape().trim().isInt(),
                body('contact', 'Enter a valid phone number. ').exists({checkFalsy:true}).isInt().isLength({min:5, max:15}),
                body('start', 'Enter a valid time. ').exists(),
                body('end', 'Enter a valid time. ').exists(),
                body('pstart', 'Enter a valid time. ').exists(),
                 body('pend').exists()
                //.custom((value, {req})=> {
