...
module.exports = {
    ...
    postForget: function(req, res, next){
        ...
        const token = jwt.sign({ email: req.body.email }, "mohit", { expiresIn: '1h' }) 
    ...
    getResetPassword: function(req, res, next){
        jwt.verify(req.params.token, "mohit", function(error, decode){
            if(error){
    ...
    putVerifyAccountAgain: function(req, res, next){
        ...
        const token = jwt.sign({ email: req.body.email }, "mohit") 
